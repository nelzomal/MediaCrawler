...
class DouYinCrawler(AbstractCrawler):
    ...
    async def search(self) -> None:
        ...
        if base_config.get_crawler_max_notes_count() < dy_limit_count:
            base_config.set_crawler_max_notes_count(dy_limit_count)
        ...
        for keyword in self.keyword.split(","):
            ...
            aweme_list: List[str] = []
            page = 0
            while (page - start_page + 1) * dy_limit_count <= base_config.get_crawler_max_notes_count():
                if page < start_page:
    ...
    async def get_specified_awemes(self):
        ...
        task_list = [
            self.get_aweme_detail(aweme_id=aweme_id, semaphore=semaphore) for aweme_id in base_config.get_dy_specified_id_list()
        ]
        aweme_details = await asyncio.gather(*task_list)
        for aweme_detail in aweme_details:
            if aweme_detail is not None:
                await douyin_store.update_douyin_aweme(aweme_detail)
        await self.batch_get_note_comments(base_config.get_dy_specified_id_list())
